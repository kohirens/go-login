...
// sharing access to the account. A secondary profile can be removed from an
// account, where a primary profile cannot because it owns the account.
...
type Profile struct {
	...
	// ClientApp list all active client apps that have logged in successfully
	// and which provider they used.
	ClientApps map[string]*ClientApp `json:"clientApp"`
	Id         string                `json:"id"`
...
func (p *Profile) FindClientApp(id string) (*ClientApp, error) {
	if ca, exists := p.ClientApps[id]; exists {
		return ca, nil
	}
...
func (p *Profile) RemoveClientApp(id string) error {
	...
	if _, exists := p.ClientApps[id]; !exists {
		return fmt.Errorf(stderr.RemoveClientApp, id, p.Name)
	}

	delete(p.ClientApps, id)
...
func profileLocation(id string) string {
	return prefixProfile + id + fileExt
}
